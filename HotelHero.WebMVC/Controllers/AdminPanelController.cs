...
namespace HotelHero.WebMVC.Controllers
{
    public class AdminPanelController : Controller
    {
        ...
        public ActionResult CreateAdditionalService()
        {
            return View();
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult CreateAdditionalService(AdditionalService additionalService)
        {
            _adminService.SaveAdditionalServiceToList(additionalService);
            return RedirectToAction("AdditionalServicesList");
        }
        public ActionResult EditAdditionalService(int id)
        {
            var model = _adminService.GetAdditionalService(id);
            return View(model);
        }
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult EditAdditionalService(AdditionalService additionalService)
        {
            _adminService.EditAdditionalService(additionalService);
            return RedirectToAction("AdditionalServicesList");
        }
        public ActionResult DeleteAdditionalService(int id)
        {
            _adminService.DeleteAdditionalService(id);
            return RedirectToAction("AdditionalServicesList");
        }
    }
}
